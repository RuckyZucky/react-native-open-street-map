...
const propTypes = {
  ...
  multiTouchControls: PropTypes.bool,
...
class MapView extends Component {
  ...
  _onMapReady() {
    const { region, initialRegion, onMapReady, zoom, multiTouchControls, zoomControls, router } = this.props;
    this.map.setNativeProps({ zoom, multiTouchControls, zoomControls, router });
    if (region) {
  ...
  takeSnapshot(args) {
    ...
    if (Platform.OS === 'android') {
      return NativeModules.OpenMapsModule.takeSnapshot(this._getHandle(), config);
    } else if (Platform.OS === 'ios') {
  ...
  render() {
    ...
    if (this.state.isReady) {
      ...
      props = {
        ...
        multiTouchControls: this.props.multiTouchControls,
        zoomControls: this.props.zoomControls,
    ...
    const OpenAIRMap = getAirMapComponent(this.props.provider);
...
const airMaps = {
  default: nativeComponent('OpenAIRMap'),
};
if (Platform.OS === 'android') {
